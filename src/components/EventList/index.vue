            ...
            <el-tabs v-model="eventActiveName">
                <el-tab-pane v-for="item in eventList" :key="item.key" :label="item.label" :name="item.key"
                    ...
                    <el-input v-if="item.key == 'redirect'" v-model="item.param" type="textarea"
                        placeholder="请输入完整的 URL" @keydown.native.stop />
                    <el-input v-if="item.key == 'alert'" v-model="item.param" type="textarea"
                        placeholder="请输入要 alert 的内容" @keydown.native.stop />
...
export default {
    ...
    methods: {
        addEvent(event, param) {
            ...
            this.$store.commit('EditPage/addEvent', { event, param })
        },

        removeEvent(event) {
            this.$store.commit('EditPage/removeEvent', event)
        },
    },
}
</script>
